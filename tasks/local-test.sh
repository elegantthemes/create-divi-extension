...
git stash pop
cd -
mv /var/create-divi-extension/patch .
git apply patch
rm patch
CMD
...
set -x
git clone /var/create-divi-extension create-divi-extension --branch ${git_branch}
cd create-divi-extension
${apply_changes}
node --version
npm --version
set +x
...
docker run \
  ...
  --tty \
  --user node \
  --volume ${PWD}/..:/var/create-divi-extension \
  --workdir /home/node \
