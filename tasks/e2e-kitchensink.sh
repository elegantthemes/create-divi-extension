...
function cleanup {
  echo 'Cleaning up.'
  unset BROWSERSLIST
  ps -ef | grep 'divi-scripts' | grep -v grep | awk '{print $2}' | xargs kill -9
  cd "$root_path"
...
cd $temp_app_path
npx create-divi-extension --internal-testing-template="$root_path"/packages/divi-scripts/fixtures/kitchensink test-kitchensink
