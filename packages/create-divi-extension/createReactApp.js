  ...
  .on('--help', () => {
    ...
    console.log(
      `      - a .tar.gz archive: ${chalk.green(
        'https://mysite.com/my-divi-scripts-0.8.2.tar.gz'
      )}`
    );
    console.log(
    ...
    console.log(
      `      ${chalk.cyan(
        'https://github.com/elegantthemes/create-divi-extension/issues/new'
      )}`
    );
    console.log();
  })
...
if (typeof projectName === 'undefined') {
  if (program.info) {
    envinfo.print({
      packages: ['react', 'react-dom', 'divi-scripts'],
      noNativeIDE: true,
      duplicates: true,
    });
    process.exit(0);
...
function getInstallPackage(version, originalDirectory) {
  let packageToInstall = 'divi-scripts';
...
function checkAppName(appName) {
  ...
  const dependencies = ['react', 'react-dom', 'divi-scripts'].sort();
