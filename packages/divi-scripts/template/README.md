...
When you run `create-divi-extension`, it always creates the project with the latest version of `divi-scripts` so you’ll get all the new features and improvements in newly created apps automatically.
...
>Note: this feature is available with `divi-scripts@0.2.0` and higher.<br>
...
The generated project includes React and ReactDOM as dependencies. It also includes a set of scripts used by Create Divi Extension as a development dependency. You may install other dependencies (for example, React Router) with `npm`:
...
>Note: this feature is available with `divi-scripts@0.5.0` and higher.
...
When you run `npm run build`, Create Divi Extension will substitute `%PUBLIC_URL%` with a correct absolute path so your project works even if you use client-side routing or host it at a non-root URL.
...
You don’t have to use [React Bootstrap](https://react-bootstrap.github.io) together with React but it is a popular library for integrating Bootstrap with React apps. If you need it, you can integrate it with Create Divi Extension by following these steps:
...
In the future we plan to integrate it into Create Divi Extension even more closely.

To learn more about Flow, check out [its documentation](https://flowtype.org/).

## Adding a Router

Create Divi Extension doesn't prescribe a specific routing solution, but [React Router](https://reacttraining.com/react-router/) is the most popular one.
...
>Note: this feature is available with `divi-scripts@0.2.3` and higher.

Your project can consume variables declared in your environment as if they were declared locally in your JS files. By
default you will have `NODE_ENV` defined for you, and any other environment variables starting with
`REACT_APP_`.

**The environment variables are embedded during the build time**. Since Create Divi Extension produces a static HTML/CSS/JS bundle, it can’t possibly read them at runtime. To read them at runtime, you would need to load HTML into memory on the server and replace placeholders in runtime, just like [described here](#injecting-data-from-the-server-into-the-page). Alternatively you can rebuild the app on the server anytime you change them.
...
>Note: this feature is available with `divi-scripts@0.9.0` and higher.
...
>Note: this feature is **available with `divi-scripts@1.0.0` and higher**.
...
>Note: this feature is available with `divi-scripts@1.1.0` and higher.
...
>Note: this feature is available with `divi-scripts@0.4.0` and higher.
...
Since Create Divi Extension doesn’t support server rendering, you might be wondering how to make `<meta>` tags dynamic and reflect the current URL. To solve this, we recommend to add placeholders into the HTML, like this:
...
>Note: this feature is available with `divi-scripts@0.3.0` and higher.<br>
>[Read the migration guide to learn how to enable it in older projects!](https://github.com/facebookincubator/create-react-app/blob/master/CHANGELOG.md#migrating-from-023-to-030)

Create Divi Extension uses [Jest](https://facebook.github.io/jest/) as its test runner. To prepare for this integration, we did a [major revamp](https://facebook.github.io/jest/blog/2016/09/01/jest-15.html) of Jest so if you heard bad things about it years ago, give it another try.

Jest is a Node-based runner. This means that the tests always run in a Node environment and not in a real browser. This lets us enable fast iteration speed and prevent flakiness.

While Jest provides browser globals such as `window` thanks to [jsdom](https://github.com/tmpvar/jsdom), they are only approximations of the real browser behavior. Jest is intended to be used for unit tests of your logic and your components rather than the DOM quirks.

We recommend that you use a separate tool for browser end-to-end tests if you need them. They are beyond the scope of Create Divi Extension.
...
Follow [this article](https://medium.com/@knowbody/circleci-and-zeits-now-sh-c9b7eebcd3c1) to set up CircleCI with a Create Divi Extension project.
...
If you use [Visual Studio Code](https://code.visualstudio.com), there is a [Jest extension](https://github.com/orta/vscode-jest) which works with Create Divi Extension out of the box. This provides a lot of IDE-like features while using a text editor: showing the status of a test run with potential fail messages inline, starting and stopping the watcher automatically, and offering one-click snapshot updates.
...
Create Divi Extension doesn't provide any built-in functionality to publish a component to npm. If you're ready to extract a component from your project so other people can use it, we recommend moving it to a separate directory outside of your project and then using a tool like [nwb](https://github.com/insin/nwb#react-components-and-libraries) to prepare it for publishing.
...
build directory. After running the build script, `create-divi-extension` will give
...
To add Source map explorer to a Create Divi Extension project, follow these steps:
...
You don’t necessarily need a static server in order to run a Create Divi Extension project in production. It works just as fine integrated into an existing dynamic one.
...
The choice of your server software isn’t important either. Since Create Divi Extension is completely platform-agnostic, there’s no need to explicitly use Node.

The `build` folder with static assets is the only output produced by Create Divi Extension.
...
By default, Create Divi Extension produces a build assuming your app is hosted at the server root.<br>
...
This will let Create Divi Extension correctly infer the root path to use in the generated HTML file.
...
>Note: this feature is available with `divi-scripts@0.2.0` and higher.
...
Create Divi Extension uses the `homepage` field to determine the root URL in the built HTML file.
...
Use the [Heroku Buildpack for Create Divi Extension](https://github.com/mars/create-react-app-buildpack).<br>
...
When you build the project, Create Divi Extension will place the `public` folder contents into the build output.
...
REACT_EDITOR | :white_check_mark: | :x: | When an app crashes in development, you will see an error overlay with clickable stack trace. When you click on it, Create Divi Extension will try to determine the editor you are using based on currently running processes, and open the relevant source file. You can [send a pull request to detect your editor of choice](https://github.com/facebookincubator/create-react-app/issues/2636). Setting this environment variable overrides the automatic detection. If you do it, make sure your systems [PATH](https://en.wikipedia.org/wiki/PATH_(variable)) environment variable points to your editor’s bin folder. You can also set it to `none` to disable it completely.
...
If you run `npm test` and the console gets stuck after printing `divi-scripts test --env=jsdom` to the console there might be a problem with your [Watchman](https://facebook.github.io/watchman/) installation as described in [facebookincubator/create-react-app#713](https://github.com/facebookincubator/create-react-app/issues/713).
...
[Ejecting](#npm-run-eject) lets you customize anything, but from that point on you have to maintain the configuration and scripts yourself. This can be daunting if you have many similar projects. In such cases instead of ejecting we recommend to *fork* `divi-scripts` and any other packages you need. [This article](https://auth0.com/blog/how-to-configure-create-react-app/) dives into how to do it in depth. You can find more discussion in [this issue](https://github.com/facebookincubator/create-react-app/issues/682).

## Something Missing?

If you have ideas for more “How To” recipes that should be on this page, [let us know](https://github.com/facebookincubator/create-divi-extension/issues) or [contribute some!](https://github.com/facebookincubator/create-divi-extension/edit/master/packages/divi-scripts/template/README.md)
